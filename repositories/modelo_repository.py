...
class ModeloRepository:
    ...
    def get_fabricante_choices(self):
        return [
            (fabricante.id, fabricante.nombre) for fabricante in Fabricante.query.all()
        ]  # Obtener opciones de fabricantes

    def get_marca_choices(self):
        return [
            (m.id, m.nombre) for m in Marca.query.filter_by(activo=True).all()
        ]  # Obtener opciones de marcas activas

    def create(self, nombre, fabricante_id, marca_id):
        nuevo_modelo = Modelo(
            nombre=nombre, fabricante_id=fabricante_id, marca_id=marca_id
        )
