...
def modificar_marcas(id):
    ...
    if form.validate_on_submit():
        if services.exists_by_name(form.nombre.data) and marca.nombre != form.nombre.data:
            form.nombre.errors.append('Esta marca ya existe.')
        else:
            services.update(id, form.nombre.data)  # Actualizar la marca usando el servicio
...
def agregar_proveedor():
    ...
    services = ProveedorService()  # Instanciar el servicio
    if form.validate_on_submit():  # Validar el formulario
        if services.exists_by_name(form.nombre.data):  # Verificar si ya existe el proveedor
            form.nombre.errors.append('Este proveedor ya existe.')
        else:
            services.create(form.nombre.data, form.telefono.data, form.direccion.data, form.correo_electronico.data)
            return redirect(url_for('proveedores'))  # Redirigir a la página de proveedores
...
def modificar_proveedor(id):
    ...
    if form.validate_on_submit():
        services.update(id, form.nombre.data, form.telefono.data, form.direccion.data, form.correo_electronico.data)
...
def modificar_modelo(id):
    ...
    if form.validate_on_submit():
        services.update(id, form.nombre.data, form.fabricante_id.data, form.marca_id.data)
...
def agregar_fabricante():
    ...
    if form.validate_on_submit():
        if service.fabricante_exists(form.nombre.data):
            ...
            service.create(form.nombre.data, form.pais_origen.data)  # Crear nuevo fabricante
...
def modificar_fabricante(id):
    ...
    if form.validate_on_submit():
        if service.fabricante_exists(form.nombre.data) and fabricante.nombre != form.nombre.data:
            form.nombre.errors.append('Este fabricante ya existe.')
        else:
            service.update(id, form.nombre.data, form.pais_origen.data)  # Actualizar fabricante
            return redirect(url_for('fabricantes'))
...
def agregar_categoria():
    ...
    services = CategoriaService()  # Instanciar el servicio
    if form.validate_on_submit():  # Validar el formulario
        if services.exists_by_name(form.nombre.data):  # Verificar si ya existe la categoría
            form.nombre.errors.append('Esta categoría ya existe.')
        else:
            services.create(form.nombre.data)  # Crear nueva categoría usando el servicio
...
def modificar_categoria(id):
    ...
    if form.validate_on_submit():
        if services.exists_by_name(form.nombre.data) and categoria.nombre != form.nombre.data:
            form.nombre.errors.append('Esta categoría ya existe.')
        else:
            services.update(id, form.nombre.data)  # Actualizar la categoría usando el servicio
...
def agregar_caracteristicas():
    ...
    services = CaracteristicaService()  # Instanciar el servicio
    form.modelo_id.choices = [(modelo.id, modelo.nombre) for modelo in services.get_active_models()]
...
def eliminar_caracteristicas():
    ...
    if request.method == 'POST':
        caracteristica_id = request.form.get('caracteristica_id')
        if services.hide_by_id(caracteristica_id):  # Ocultar característica usando el servicio
...
def agregar_accesorio():
    ...
    services = AccesorioService()  # Instanciar el servicio
    if form.validate_on_submit():
        if services.exists_for_modelo(form.modelo_id.data):  # Verificar si ya existe el accesorio
...
def modificar_accesorio(id):
    ...
    if form.validate_on_submit():
        if services.exists_for_modelo(form.modelo_id.data) and accesorio.modelo_id != form.modelo_id.data:
            form.modelo_id.errors.append('Este accesorio ya existe para el modelo seleccionado.')
        else:
            services.update(id, 
                modelo_id=form.modelo_id.data,
...
def modificar_equipos(id):
    ...
    if form.validate_on_submit():
        if services.get_by_id(form.modelo_id.data) and equipo.modelo_id != form.modelo_id.data:
            form.modelo_id.errors.append('Este equipo ya existe para el modelo seleccionado.')
        else:
...
def agregar_inventario():
    ...
    proveedores = services.get_proveedores()  # Método que obtiene los proveedores activos
...
def modificar_inventario(id):
    ...
    if request.method == 'POST':
        ...
        services.update(id, nuevo_nombre, nuevo_proveedor_id, nueva_cantidad, nueva_fecha)
