...
def modelos():
    ...
    if request.method == "PUT":
        ...
        return (
            jsonify({"Mensaje": "UD no está habilitado para actualizar un usuario."}),
            403,
        )
    ...
    if request.method == "DELETE":
        if administrador:
            ...
            if not modelo_id:
                return (
                    jsonify({"Mensaje": "Falta el parámetro 'id' en la solicitud"}),
                    400,
                )  # error si falta el ID

            modelo = Modelo.query.get(modelo_id)
            if not modelo:
                return (
                    jsonify({"Mensaje": "Modelo no encontrado"}),
                    404,
            ...
            db.session.commit()
            return (
                jsonify({"Mensaje": "Modelo marcado como inactivo correctamente."}),
                200,
            )
        return (
            jsonify(
                {
                    "Mensaje": "UD no está habilitado para cambiar el estado de este modelo."
                }
            ),
            403,
        )
