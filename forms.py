...
from wtforms.validators import (
    ...
    Length,
    Email,
)  # bibliotecas externas a Flask-WTF, pero se usa junto con Flask-WTF
...
class ProveedorForm(FlaskForm):
    ...
    correo_electronico = StringField(
        "Correo Electrónico", validators=[DataRequired(), Email()]
    )
...
class ModeloForm(FlaskForm):
    ...
    marca_id = SelectField(
        "Marca", coerce=int, validators=[DataRequired()]
    )  # Añadido aquí
    submit = SubmitField("Guardar")
...
class CaracteristicaForm(FlaskForm):
    ...
    resolucion_pantalla = StringField(
        "Resolución de Pantalla", validators=[Length(max=50)]
    )
    ...
    descripcion = StringField(
        "Descripción", validators=[DataRequired(), Length(max=200)]
    )
...
class EquipoForm(FlaskForm):
    ...
    modelo_id = SelectField("Modelo", coerce=int, validators=[DataRequired()])
    caracteristica_id = SelectField(
        "Precio dólar - Batería en horas", coerce=int, validators=[DataRequired()]
    )
    accesorio_id = SelectField(
        "Cargador - Auriculares", coerce=int, validators=[DataRequired()]
    )
    submit = SubmitField("Guardar")
