...
class Fabricante(db.Model):
    ...
    modelos = db.relationship('Modelo', back_populates='fabricante') #back_populates crea una relacion bidireccional entre Fabricante y Modelo, un fabricante puede tener muchos modelos
...
class Modelo(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    nombre = db.Column(db.String(50), nullable=False)
    fabricante_id = db.Column(db.Integer, db.ForeignKey('fabricante.id'), nullable=False) #acá se almacena el id del fabricante, pero no proporciona una representación descriptiva o legible del fabricante 
    marca_id = db.Column(db.Integer, db.ForeignKey('marca.id'), nullable=False)  # Añadido aquí
    activo = db.Column(db.Boolean, default=True)  # Añade el atributo activo
...
class Caracteristica(db.Model):
    ...
    def __str__(self) -> str:
        return self.peso, self.resolucion_pantalla, self.capacidad_bateria, self.sistema_operativo, self.precio_lista, self.activo, self.camara, self.descripcion, self.lanzamiento 
...
class Inventario(db.Model):
    ...
    proveedor = db.relationship('Proveedor', backref=db.backref('inventario', lazy=True))
